    ...
    const cards = [{
      ...
      function handleSubmit(e:FormEvent){
        ...
        if(dataEntries !== undefined){
            ...
            const existingData = props.battlefield.find(card => card.name === data.name)
    ...
    return(
        <form onSubmit={handleSubmit} className="inline cardForm">
            <select className="bg-inherit" name="chosenCard">
                {cards.map((card)=>(<option className="text-black" value={JSON.stringify(card)}>{card.name}</option>))}
            </select>
